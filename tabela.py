...
def inserir_frase():
    ...
    if frase.strip() == "":
        messagebox.showerror("Erro", "Digite uma frase válida!")
        return
...
def visualizar_frases():
    ...
    if not frases:
        ...
        for frase in frases:
            lista_frases.insert(END, f"{frase[0]}: {frase[1]}")
...
def exportar_para_word():
    ...
    documento = Document()
    documento.add_heading('Tabela de Frases', 0)
    ...
    for frase in frases:
        ...
        row_cells[0].text = str(frase[0])
        row_cells[1].text = frase[1]
...
Label(frame_inserir_visualizar, text="Digite uma nova frase:").grid(row=0, column=0, padx=5, pady=5)
entrada_frase = Entry(frame_inserir_visualizar, width=50)
entrada_frase.grid(row=0, column=1, padx=5, pady=5)

botao_inserir = Button(frame_inserir_visualizar, text="Inserir Frase", command=inserir_frase)
botao_inserir.grid(row=0, column=2, padx=5, pady=5)

Label(frame_inserir_visualizar, text="ID da frase a ser deletada:").grid(row=1, column=0, padx=5, pady=5)
...
botao_deletar.grid(row=1, column=2, padx=5, pady=5)

botao_visualizar = Button(frame_inserir_visualizar, text="Visualizar Frases", command=visualizar_frases)
botao_visualizar.grid(row=2, column=1, padx=5, pady=5)

# Frame para exportar frases para o Word
frame_exportar = Frame(root)
frame_exportar.pack(padx=10, pady=10)

botao_exportar_word = Button(frame_exportar, text="Exportar Frases para Word", command=exportar_para_word)
botao_exportar_word.pack()
